...
async def kb_answer(message: types.Message):
    ...
    if (quest == 5) and (choose == "Нашёл"):
        where = message.text  
        await message.answer("обратитесь в 404 кабинет для того чтобы оставить там находку")
        print(choose, item_discr, "в", where, when)
        data = {
            "info": info,
    ...
    if (quest == 5) and (choose == "Потерял"):
        #написать цикл для отправления всех потерянных вещей
        pass
